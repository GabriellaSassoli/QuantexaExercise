...
import quantexa.services.Question2.{calculateAverage, exerciseSolver, getTransactionsByUserAndCategory}

import scala.collection.immutable.HashMap

class Question2Test extends FlatSpec with Matchers {

  "exerciseSolver" should "return the average transaction grouped by userID and Category" in {
    val input = readInput("transactionTest.txt")
    exerciseSolver(input) shouldBe HashMap("A27" -> HashMap("BB" -> 0.0, "FF" -> 0.0, "GG" -> 338.11), "A39" -> HashMap("BB" -> 243.39, "FF" -> 0.0, "GG" -> 0.0), "A13" -> HashMap("BB" -> 0.0, "FF" -> 516.34, "GG" -> 0.0))
  }
