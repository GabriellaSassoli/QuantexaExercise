...
class Question3Test extends FlatSpec with Matchers {
  ...
  "calcStats" should "return a map of statics by userID" in {
    ...
    calcStats(input,6) shouldBe Map("A27" -> CalculatedStatistics(3.0,2.5,0.0,0.0,0.0), "A13" -> CalculatedStatistics(516.34,516.34,0.0,0.0,516.34))
    calcStats(input,3) shouldBe Map("A13" -> CalculatedStatistics(516.34,516.34,0.0,0.0,516.34))
